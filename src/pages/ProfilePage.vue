...
const userProperties = computed(() => {
  return propertiesStore.properties.filter((a: Property) => a.owner === userStore.User.address || a.hirer === userStore.User.address);
});
...
const isNameValid = computed(() => {
  return property.value.name.length > 5;
});
...
const isAddressValid = computed(() => {
  return property.value.adres.length > 5;
});

const isPriceValid = computed(() => {
  return property.value.price > 5;
});
